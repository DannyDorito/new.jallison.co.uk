...
describe('When the page is rendered', () => {
  it('should render the <Education /> JSX component', async () => {
    ...
    expectedEducation.forEach(async (expected) => {
      expect(await screen.findByText(expected.institution)).toBeInTheDocument();

      expect(await screen.findByText(expected.qualification)).toBeInTheDocument();
      ...
      expect(
        await screen.findByText(
          `${expected.startDate.toLocaleString('default', {
            month: 'long',
          })} ${expected.startDate.getFullYear()} - ${expected.endDate.toLocaleString('default', {
            month: 'long',
